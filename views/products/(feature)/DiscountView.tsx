...
import { Breadcrumb, Button, Container, DiscountTag, QuantitySelector, StarRating } from '@/components'
...
const DiscountView = () => {
    ...
    const { title, price, description, oldPrice, sizes = [], images = [], colors = [], category, ratingAverage,
        ratingTotal, stock, sku, deliveryEstimate, vendor, discounts } = product || {};
    ...
    // APPY DISOUNT
    const handleDiscountApply = (discount: DiscountType) => {
    if (appliedDiscount?.id === discount.id) {
        setAppliedDiscount(null); // unapply if already applied
    } else {
    ...
    const discountedPrice = appliedDiscount
      ? price - (price * appliedDiscount.percentage) / 100
      : price;
                            ...
                            <div className="grid lg:grid-cols-2 gap-10">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-2">
                                      <h2 className="text-sm font-medium">Coupon discount Codes</h2>
                                      ...
                                      <div className="flex items-center gap-3 flex-wrap">
                                          {discounts?.map((discount: DiscountType, index: number) => (
                                              <DiscountTag
                                                  ...
                                                  label={appliedDiscount?.id === discount.id ? 'X' : 'Get it'}
                                                  onClick={() => handleDiscountApply(discount)}
                                              />
                                          ))}
                                      </div>
                                      ...
                                      <div className="items-center flex gap-x-3 text-base md:text-lg font-medium">
                                        {appliedDiscount ? (
                                          <>
                                            ...
                                            <p className="text-gray-400 line-through">{formatCurrency(price)}</p>
                                          </>
                                        ) : (
