...
const MegaMenuContainer = ({ type, onLinkClick }: { type?: MegaMenuType, onLinkClick?: () => void }) => {
    ...
    return (
        <div className={`h-fulll overflow-y-scroll max-h-[77vh] px-4 py-8 mt-0.5 absolute duration-300 ease-in-out transition-all
        ${open ? 'opacity-100 translate-y-0 pointer-events-auto visible' : 'opacity-0 -translate-y-2 pointer-events-none invisible'}
        xl:block top-full right-0 overflow-hidden bg-white dark:bg-gray-800 mx-auto left-0 z-20 p-2`}>
