...
const BlogCard = ({ post, variant }: { post: PostType; variant?: PostVariantType }) => {
    ...
    return (
        <Link href={`/blog/${id}`} className={`group duration-200 ${variant === 'detailed' ? 'space-y-3' : 'space-y-2'}`}>
            ...
            {variant === 'detailed' && (
                <h1 className="font-semibold text-sm lg:text-base line-clamp-2 
            ...
            <Image
                ...
                width={800}
                height={400}
                className="object-cover w-full aspect-[7/4] rounded-lg"
            />
            ...
            {variant === 'detailed' && date && (
                <div className="flex items-center flex-wrap gap-3 justify-between text-black dark:text-gray-200">
            ...
            {variant === 'compact' && (
                <>
                    <p className="text-sm line-clamp-2 text-black dark:text-gray-300">{description}</p>

                    <div className="flex items-center flex-wrap gap-y-3 gap-x-6 text-black dark:text-gray-200">
                        <div className="inline-flex items-center gap-2 text-sm">
                            <UserCircleIcon className="size-5" />
                            <span className="font-medium">By {author}</span>
                        </div>
            ...
            {variant === 'detailed' && (
                <p className="text-sm line-clamp-3 text-black dark:text-gray-300">{description}</p>
            )}
        </Link>
    );
};
