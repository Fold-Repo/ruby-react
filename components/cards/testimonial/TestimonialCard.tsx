...
const TestimonialCard = ({ testimonial, active }: { testimonial: TestimonialType; active: boolean }) => {
    ...
    return (
        <div className={`p-5 md:p-6 !rounded-lg space-y-3 transition-all duration-300 
            ${active ? 'bg-primary text-white' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200'}`}
            style={{ boxShadow: '0px 7px 30px 0px #0000000D' }}>

            <RiDoubleQuotesL className={`text-3xl ${active ? 'text-white' : 'text-gray-500 dark:text-gray-400'}`} />

            <h2 className="text-base 2xl:text-lg font-bold"> {name} </h2>

            <p className={`${active ? 'text-white' : 'text-gray-600 dark:text-gray-300'} text-sm/6`}>
                {message}
            </p>
